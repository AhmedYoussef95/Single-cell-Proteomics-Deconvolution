...
forPlot <- geneLambdas %>% 
  ...
  arrange(Number_of_clusters) %>% 
  mutate(Number_of_clusters = factor(paste(Number_of_clusters, "clusters")), Lambda = as.numeric(Lambda),
         Cross_cluster_variation = factor(Cross_cluster_variation, levels = c("Low variation", "Medium variation", "High variation")))
...
ggplot(forPlot) +
  ...
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Variation of gene's expression across clusters")
...
```{r computeLambdaForDifferentGenesBasedOnVariabilityTimepoints, eval = FALSE}
#list of resolutions for different numbers of seurat clusters
clusters <- setNames(c(0.2,0.3,0.4,0.5,0.7,0.9,1.0,1.32,1.35,1.4,1.8), 5:15)
...
#compute distance between real matrix and predicted matrix for different number of Seurat clusters (~11m)
timeGeneLambdas <- pblapply(names(clusters), function(clust){
  #cluster seurat object
  seurat <- FindClusters(seurat, resolution = clusters[clust], random.seed = 123)
  ...
  #compute coefficient of variation of genes across clusters
  geneCV <- apply(Y, 2, function(x) return(sd(x) / mean(x)))
  ...
  #create table with each gene's variation classification
  geneVarClass <- do.call("rbind", lapply(names(geneCV), function(x) return(data.frame(Gene = names(geneCV[[x]]), Cross_timepoint_variation = x))))
...
#merge all results into one table
# get all variable names
allNms <- unique(unlist(lapply(timeGeneLambdas, names)))
timeGeneLambdas <- do.call(rbind, c(lapply(timeGeneLambdas, function(x) data.frame(c(x, sapply(setdiff(allNms, names(x)), function(y) NA)))), make.row.names=FALSE))

timeGeneLambdas$Lambda <- as.numeric(timeGeneLambdas$Lambda)
timeGeneLambdas$Number_of_clusters <- as.numeric(timeGeneLambdas$Number_of_clusters)
saveRDS(timeGeneLambdas, "/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/crossTimepointGeneLambdas.rds")
```

```{r lambdaForDifferentGenesCrossTimepointVariability, fig.width = 16, fig.height = 16}
#read in all scores for all values of lambda and all genes
timeGeneLambdas <- readRDS("/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/crossTimepointGeneLambdas.rds")

#keep optimal lambda for each gene
forPlot <- timeGeneLambdas %>% 
  group_by(Gene, Number_of_clusters) %>% 
  filter(Relative_error == min(Relative_error)) %>% 
  arrange(Number_of_clusters) %>% 
  mutate(Number_of_clusters = factor(paste(Number_of_clusters, "clusters")), Lambda = as.numeric(Lambda),
         Cross_timepoint_variation = factor(Cross_timepoint_variation, levels = c("Low variation", "Medium variation", "High variation")))

#plot
ggplot(forPlot) +
  geom_boxplot(aes(x = Cross_timepoint_variation, y = log(Lambda), fill = Cross_timepoint_variation)) +
  #scale_y_continuous(limits = quantile(log(geneLambdas$Lambda), c(0.1,0.9))) +
  facet_wrap( ~ Number_of_clusters) + 
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Variation of gene's expression across timepoints")
