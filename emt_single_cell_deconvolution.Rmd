...
```{r geneVarWithinCluster, out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
#list of resolutions for different numbers of seurat clusters
clusters <- setNames(c(0.2,0.3,0.4,0.5,0.7,0.9,1.0,1.32,1.35,1.4,1.8), 5:15)
...
ggplot(errors, aes(x = log(Lambda), y = Sum_squared_error, col = Number_of_clusters)) +
  ...
  labs(title = "Error of estimating single-cell profiles using ridge regression for different values of lambda",
...
```{r compareRidgeRegressionToOLS, out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
...
ggplot(errors, aes(x = Number_of_clusters, y = `Relative RMSE`, col = Method)) +
  ...
  labs(title = "Relative RMSE of estimating single-cell profiles using ridge regression and OLS for different numbers of clusters")
```
