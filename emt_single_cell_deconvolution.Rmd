...
#compute distance between real matrix and predicted matrix for different number of Seurat clusters (~11m)
geneLambdas <- pblapply(names(clusters), function(clust){
  ...
  geneCV_clusters_groups <- split(sort(geneCV_clusters), sort(1:length(geneCV_clusters) %% 30))
  ...
  #group genes into groups according to variance across clusters
  geneCV_timepoints_groups <- split(sort(geneCV_timepoints), sort(1:length(geneCV_timepoints) %% 30))
  ...
  av_expression_groups <- split(sort(av_expression), sort(1:length(av_expression) %% 30))
  ...
  geneLogFC_groups <- split(sort(geneLogFC), sort(1:length(geneLogFC) %% 30))
  ...
  geneVarClass <- list(clusterCVClass, timepointCVClass, avExpressionClass, geneLogFcClass) %>% purrr::reduce(left_join, by = "Gene")
...
geneLambdas$Max_Log_FC <- as.numeric(geneLambdas$Max_Log_FC)
geneLambdas$Cross_cluster_CV <- as.numeric(geneLambdas$Cross_cluster_CV)
geneLambdas$Cross_timepoint_CV <- as.numeric(geneLambdas$Cross_timepoint_CV)
geneLambdas$Number_of_clusters <- as.integer(geneLambdas$Number_of_clusters)
geneLambdas$Cross_cluster_CV_class <- as.integer(geneLambdas$Cross_cluster_CV_class)
geneLambdas$Cross_timepoint_CV_class <- as.integer(geneLambdas$Cross_timepoint_CV_class)
geneLambdas$Average_expression_class <- as.integer(geneLambdas$Average_expression_class)
geneLambdas$Log_FC_class <- as.integer(geneLambdas$Log_FC_class)
...
forPlot <- geneLambdas %>% 
  ...
  mutate(Number_of_clusters = factor(paste(Number_of_clusters, "clusters")), Lambda = as.numeric(Lambda))

#plot
ggplot(forPlot) +
  geom_boxplot(aes(x = factor(Cross_cluster_CV_class), y = log(Lambda)), fill = "skyblue") +
  ...
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Variation of gene's expression across clusters", x = "Cross cluster CV") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```

We repeat the same process as above, except this time grouping the genes based on their variability across **timepoints** in the reconstructed pseudobulk data.


```{r lambdaForDifferentGenesCrossTimepointVariability, fig.width = 16, fig.height = 16}
#read in all scores for all values of lambda and all genes
#geneLambdas <- readRDS("/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/geneLambdas.rds")
...
#plot
ggplot(forPlot) +
  geom_boxplot(aes(x = factor(Cross_timepoint_CV_class), y = log(Lambda)), fill = "skyblue") +
  #scale_y_continuous(limits = quantile(log(geneLambdas$Lambda), c(0.1,0.9))) +
  #scale_y_continuous("log(Lambda)", labels = paste0("log(", as.character(geneLambdas$Lambda), ")"), breaks = log(geneLambdas$Lambda)) +
  facet_wrap( ~ Number_of_clusters) + 
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Variation of gene's expression across timepoints", x = "Cross timepoint CV") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```

We repeat the same process as above, except this time grouping the genes based on the magnitude of their average expression across timepoints.


```{r lambdaForDifferentGenesAverageTimepointExpression, fig.width = 16, fig.height = 16}
#plot
ggplot(forPlot) +
  geom_boxplot(aes(x = factor(Average_expression_class), y = log(Lambda)), fill = "skyblue") +
  #scale_y_continuous(limits = quantile(log(geneLambdas$Lambda), c(0.1,0.9))) +
  #scale_y_continuous("log(Lambda)", labels = paste0("log(", as.character(geneLambdas$Lambda), ")"), breaks = log(geneLambdas$Lambda)) +
  facet_wrap( ~ Number_of_clusters) + 
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Average of gene's expression across timepoints", x = "Average expression") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```
We repeat the same process as above, except this time grouping the genes based on the maximum cross-cluster log-fold change. The log fold-change for each gene in each cluster was computed using *Seurat*'s $FindAllMarkers()$ function. The final retained log fold-change is the maximum one for the gene across clusters.

```{r lambdaForDifferentGenesAverageLogFoldChange, fig.width = 16, fig.height = 16}
#plot
ggplot(forPlot) +
  geom_boxplot(aes(x = factor(Log_FC_class), y = log(Lambda)), fill = "skyblue") +
  #scale_y_continuous(limits = quantile(log(geneLambdas$Lambda), c(0.1,0.9))) +
  #scale_y_continuous("log(Lambda)", labels = paste0("log(", as.character(geneLambdas$Lambda), ")"), breaks = log(geneLambdas$Lambda)) +
  facet_wrap( ~ Number_of_clusters) + 
  labs(title = "How does the optimal regression parameter vary for different categories of genes?",
       subtitle = "Log fold-change in highest expressed cluster compared to other clusters", x = "Max log fold-change") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```
...
#compute distance between real matrix and predicted matrix for different number of Seurat clusters and different bulk datasets
...
# Modified Algorithm - tuning lambda to individual genes

This section introduces a modification to the algorithm by tuning the regression parameter $\lambda$ to each individual gene's prediction. For each gene, we test the prediction accuracy of 21 values of $\lambda$ (ranging from 1e−10 to 1e10) for deconvoluting reconstructed pseudobulk RNA data to the single-cell level, with the scRNA data being the benchmark value. The value of $\lambda$ that minimized the error to the scRNA profiles is retained as the optimal value.

After selecting the optimal $\lambda$ for each gene and each number of clusters we report the errors as relative RMAD (relative mean absolute deviation) using the formula $|X - X'| / X$, where $|X|$ is the absolute value of the difference. This error is computed for each gene and the final reported score is the average RMAD value, in other words: *on average, how different is a gene's predicted values compared to the true ones?* The resultant relative RMAD values are shown below.
...
optimalLambdas <- geneLambdas %>% 
  group_by(Gene, Number_of_clusters, Dataset) %>% 
  filter(Relative_error == min(Relative_error)) %>% 
...
errors <- optimalLambdas %>%
  ...
  group_by(Number_of_clusters, Dataset) %>% 
...
#for each no. of clusters, get each gene's error, specificity, and average expression
predictions <- pblapply(names(clusters), function(clust){
  #get error values for current no. of clusters
  curErrors <- optimalLambdas[optimalLambdas$Number_of_clusters == as.numeric(clust) & optimalLambdas$Dataset == "Reconstructed pseudobulk",]
...
```{r hallmarkHeatmapsPredictions, fig.width = 12}
#read in EMT hallmark genes
hallmark <- read.delim("/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/hallmark_emt_geneset.txt", header = F)[,1]
...
```
## Applying algorithm to different datasets
...
2) ***Reconstructed Pseudo-bulk***: The reconstructed pseudo-bulk is created by matrix multiplication of the average cell cluster expression of each gene and the number of cells in each cluster each timepoint, i.e. $Y = AX$.
3) ***Bulk mRNA***: Bulk mRNA data from a microarray experiment.
4) ***Proteomics***: Bulk proteomics data from a mass spectrometry experiment.

For each of the above datasets, we apply the deconvolution algorithm and compute the prediction error as the relative error between our predicted cell-cluster matrix $X'$ and the one obtained from the Seurat clustering of the scRNA data $X$. The regression paramter $\lambda$ was tuned for each gene in each dataset separately. Since the identified genes differ across the three technologies, we retain only the ~5,000 genes common to all datasets for this comparison.

Note: Moving forward, the optimal $\lambda$ values used for deconvoluting all the datasets is that derived from deconvoluting the reconstructed pseudobulk data.

```{r applyDeconvolutionToDifferentBulkDatasetsIndividualLambdas,  out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
#read in all scores for all values of lambda and all genes
#geneLambdas <- readRDS("/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/geneLambdas.rds")
...
#visualize errors with line plots
ggplot(errors, aes(x = Number_of_clusters, y = Relative_error, color = Dataset)) +
  geom_line() + geom_point() +
  scale_x_continuous(breaks = 5:15) +
  labs(title = "Relative error of estimating single-cell profiles for different numbers of clusters")
```
...
# Comparison to single-cell proteomics data

As an external benchmark, the Nikolai Slavov group at Northeastern University shared with us an unpublished MS-derived single-cell proteomics dataset from cells undergoing EMT. Their experimental setup consists of three timepoints, and the data has 1,827 genes x 420 cells, out of which 1,064 genes, including 27 of the hallmark genes, were also detected in our own data. In this section, we compare those experimentally-derived single-cell profiles to our computationally-predicted ones as an external benchmark.

## Data Pre-processing

For all downstream analysis, only the set of ~1,000 genes that overlap between the single-cell proteomics, bulk proteomics, and scRNA-Seq datasets are retained. 65% of the original single-cell proteomics matrix consists of missing values which were imputed by k-nearest neighbor imputation (k = 3) using Euclidean distance as a similarity measure between the cells following the [Specht et al](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02267-5#Sec16) publication.
...
preprocessSCProteomics <- function(scProt, k = 3, distanceMetric = c("euclidean", "pearson")){
...
#pre-process data
scProt <- preprocessSCProteomics(scProt, k = 3, distanceMetric = "euclidean")
```

The proteins detected only in the single-cell proteomics data showed significant enrichment ($FDR < 0.05$) of the following GO terms:

```{r scOnlyProteinsEnrichment, message = FALSE}
...
## Identify cell clusters across datasets

First, the cell states in the single-cell proteomics dataset are identified in an unsupervised manner based on similarity of protein abundance profiles. All cells from all timepoints are pooled together for this analysis. [Seurat](https://www.nature.com/articles/nbt.4096) is used to cluster the cells based on the 2,000 most variable proteins following their standard pipeline for scRNA-Seq data. Our model defines 11 cell clusters, to be comparable to the clusters identified in the scRNA-Seq data, by using a Seurat clustering resolution of 2.5 in the proteomics data and 1.2 in the RNA data. The below UMAP plots visualize the identified cell clusters.

```{r reconstructBulkProteomicsFromSCproteomics, fig.width = 10}
#create seurat objects
seuratProt <- createSeurat(as.matrix(scProt))
seuratRNA <- createSeurat(as.matrix(scrna[overlapProts,]))

#cluster both to 11 clusters
seuratProt <- FindClusters(seuratProt, resolution = 2.5, random.seed = 123)
seuratRNA <- FindClusters(seuratRNA, resolution = 1.2, random.seed = 123)

#UMAP dimensionality reduction
seuratProt <- RunUMAP(seuratProt, dims = 1:10)
seuratRNA <- RunUMAP(seuratRNA, dims = 1:10)

#plot UMAP
p1 <- DimPlot(seuratProt, reduction = "umap", label = T, pt.size = 3) + ggtitle("SC Proteomics cell clusters")
...
umap$timepoint <- colnames(seuratProt)
umap$timepoint <- gsub(".*_","", umap$timepoint)

#create matrix A with counts of cell clusters in each timepoint
A_prot <- computeCellProportions(seuratProt)
...
#add column for cluster name
timeProportions_prot$Cell_Type <- as.factor(rep(0:(length(unique(Idents(seuratProt)))-1), length(unique(timeProportions_prot$Timepoint))))

#plot
p1 <- ggplot(timeProportions_prot, aes(x = Timepoint, y = Proportion, fill = Cell_Type,
                                       label = ifelse(Proportion > 0.015, paste0(Proportion*100,'%'), "") )) +
  geom_bar(stat = 'identity') + 
  geom_text(size = 2, position = position_stack(vjust = 0.5)) +
...
#convert cell type counts to proportions
timeProportions_rna <- data.frame(apply(A_rna, 1, function(x) return(x/sum(x))))

#convert to long format for ggplot
timeProportions_rna <- melt(timeProportions_rna)
colnames(timeProportions_rna) <- c('Timepoint', 'Proportion')
timeProportions_rna$Proportion <- round(timeProportions_rna$Proportion, 2)

#add column for cluster name
timeProportions_rna$Cell_Type <- as.factor(rep(0:(length(unique(Idents(seuratProt)))-1), length(unique(timeProportions_rna$Timepoint))))

#plot
p2 <- ggplot(timeProportions_rna[timeProportions_rna$Timepoint %in% c("T0", "T4", "T8"),], aes(x = Timepoint, y = Proportion, fill = Cell_Type,
                                                                                               label = ifelse(Proportion > 0.015, paste0(Proportion*100,'%'), "") )) +
  geom_bar(stat = 'identity') + 
  geom_text(size = 2, position = position_stack(vjust = 0.5)) +
  ggtitle("Cell type composition in each timepoint - SC RNA")

p1 | p2

#merge UMAP plots
scUMAP <- seuratProt@reductions$umap@cell.embeddings %>% as.data.frame() %>% mutate(cluster = paste0("proteomics_", Idents(seuratProt)),
                                                                                    data ="SC proteomics", timepoint = gsub(".*_", "", colnames(scProt)))
rnaUMAP <- seuratRNA@reductions$umap@cell.embeddings %>% as.data.frame() %>% mutate(cluster = paste0("rna_", Idents(seuratRNA)),
                                                                                    data = "scRNA", timepoint = gsub(".*_", "", colnames(scrna)))
mergedUMAP <- rbind(scUMAP, rnaUMAP)

#plot merged UMAP
ggplot(mergedUMAP, aes(x = UMAP_1, y = UMAP_2, color = cluster, shape = data)) +
  geom_point(size = 2) +
  labs(title = "Combined UMAP of SC Proteomics and scRNA cells")
```
...
X_prot <- as.data.frame(scProt[overlapProts,]) %>% 
  mutate(gene = rownames(.)) %>% 
  melt() %>% 
  mutate(cluster = Idents(seuratProt)[variable]) %>% 
  group_by(gene, cluster) %>% 
  summarise(value = mean(value)) %>% 
  dcast(gene ~ cluster) %>% 
  magrittr::set_rownames(.$gene) %>% 
...
#predicted profiles
X_prediction <- t(prediction)

#overlap
predictionOverlap <- intersect(rownames(X_prot), rownames(X_prediction))
X_prot <- X_prot[predictionOverlap,]
X_prediction <- X_prediction[predictionOverlap,]

#rename cluster centroids to indicate which dataset they came from
colnames(X_prot) <- paste0("sc_", colnames(X_prot))
colnames(X_prediction) <- paste0("prediction_", colnames(X_prediction))
...
pheatmap(X_prediction, scale = "row",
         show_rownames = F, show_colnames = T,
         cluster_cols = F, cluster_rows = F, 
         main = "Predicted proteomics cell cluster profiles (scaled by row)")
```


The below UMAP plot includes the centroid of each cluster in each dataset. The centroid is computed as the average abundance of each protein in each cluster's cells. Prior to performing the UMAP dimensionality reduction, the values were normalized to sum to 1 in each cluster.

```{r UMAPcombinedCentroids}
#merge centroids to one table
mergedCentroids <- cbind(X_prot, X_prediction)
...
#keep overlapping genes only
bulkProt <- bulkProt[predictionOverlap,]
pseudobulkProt <- pseudobulkProt[predictionOverlap,]
reconstructedProt <- reconstructedProt[predictionOverlap,]
...
## Comparing cell clusters across datasets
...
```{r comparingClusters}
#correlate clusters
columnCor <- lapply(seq(ncol(X_prot)), function(i){
  curPrediction <- X_prediction[,i]
  clusterCors <- list()
  for(j in seq(ncol(X_prot))){
    curProt <- X_prot[,j]
  ...
  do.call("rbind", .)
rownames(columnCor) <- colnames(X_prediction)
colnames(columnCor) <- colnames(X_prot)
pheatmap(columnCor, main = "Pearson correlation of cluster centroids")

#columnCor <- unlist(lapply(columnCor, max))
```
