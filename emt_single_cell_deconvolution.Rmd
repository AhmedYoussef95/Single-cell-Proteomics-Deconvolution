...
![](/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Plots/bulk decnovolution outline.png)
...
#function to compute cell-type proportions 'A' given scRNA data and seurat clustering resolution
computeCellProportions <- function(seurat, resolution){
  #create list of timepoints
  timepoints <- unique(gsub(pattern = ".*_", replacement = "", colnames(seurat)))
  
  #create vector which contains the timepoint each cell comes from
  cellTimepoints <- as.factor(gsub(pattern = '.*_', replacement = "", colnames(seurat)))
...
```{r predictSingleCellProfilesFromBulkAndTimeProportions, message=FALSE, results='hide', out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
...
errors <- lapply(clusters, function(clust){
  ...
  A <- computeCellProportions(seurat, resolution = clust)
...
#plot no. of clusters to error
plot(5:15, unlist(errors), main = "Re-creating single-cell data from pseudo-bulk", xlab = "Number of clusters", ylab = "RMSE", 'b')
```
\newpage

The process above is repeated but this time using $Y'$ as the bulk data, where $Y' = AX$ is computed first prior to computing $X'$ as above.

```{r predictSingleCellFromYhat, results='hide', out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
#compute distance between real matrix and predicted matrix for different number of Seurat clusters
...
#plot no. of clusters to error
plot(5:15, unlist(errors2), main = "Re-creating single-cell data from pseudo-bulk (Y' = AX)", xlab = "Number of clusters", ylab = "RMSE", 'b')
```

Finally, we repeat the above process using the actual bulk data in this dataset.

```{r predictSingleCellFromActualBulk, results='hide', out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold'}
#compute distance between real matrix and predicted matrix for different number of Seurat clusters
...
#plot no. of clusters to error
plot(5:15, unlist(errors3), main = "Re-creating single-cell data from actual bulk", xlab = "Number of clusters", ylab = "RMSE", type = 'b')
```

The above plots are now shown on the same plot below.

```{r comparingSCpredictionErrorsDifferentBulkData}
...
matplot(t(allErrors), type = c("b"), pch = 1, col = 1:3, xlab = "Number of clusters", ylab = "RMSE", x = 5:15)
