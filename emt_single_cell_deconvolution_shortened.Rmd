...
results <- pblapply(names(clusters), function(clust){
  ...
  #predict single-cell profiles using constrained ridge regression for each value of lambda (~1m)
...
Here, we examine an alternative clustering of the data, which clustered the scRNA data into 20 clusters using the [Monocle3 algorithm](https://cole-trapnell-lab.github.io/monocle3/). Note: the regression parameter $\lambda$ chosen was the same one used for the Seurat clustering in the previous section.
...
pheatmap(t(prediction_hallmark), scale = "row", show_rownames = T, cluster_cols = F, main = "Single-cell proteomics prediction - Monocle 20 clusters - EMT hallmark genes", cluster_rows = F)
