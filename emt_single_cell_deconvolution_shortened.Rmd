...
Here, we examine an alternative clustering of the data, which clustered the scRNA data into 20 clusters using the [Monocle3 algorithm](https://cole-trapnell-lab.github.io/monocle3/). Note: the regression parameters $\lambda$ chosen was the same one used for the Seurat clustering in the previous section.

```{r monocleClustersIndividualLambdas, out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold', fig.height = 8, fig.width = 12}
...
#add column for cluster name
timeProportions$Cell_Type <- as.factor(rep(1:(length(unique(monocle))), ncol(mrna)))
...
\newpage
```{r deconvoluteProteomicsMonocleIndividualLambdas, out.width = '.49\\linewidth', fig.align = 'center', fig.show = 'hold', fig.height = 12}
#construct matrix with cell-type expression of each gene
X <- scrna %>% 
  mutate(gene = rownames(.)) %>% 
  melt() %>% 
...
#predict single-cell profiles using constrained ridge regression
prediction <- t(perGeneConstrainedRidgeRegression(bulk = bulk_proteomics, timeProportions = A, optimalLambdas = optimalLambdas))
...
hallmark <- read.delim("/Users/Ahmed/Documents/Emili_Lab/Aim 3/EMT Multi-omics/Data/hallmark_emt_geneset.txt", header = F)[,1]
hallmarkOverlap <- intersect(overlapGenes, hallmark)
prediction_hallmark <- prediction[intersect(rownames(prediction), hallmarkOverlap),]

#visualize
pheatmap(prediction, scale = "row", show_rownames = F, cluster_cols = F, main = "Single-cell proteomics prediction - Monocle 20 clusters - all genes")
pheatmap(prediction_hallmark, scale = "row", show_rownames = T, cluster_cols = F, main = "Single-cell proteomics prediction - Monocle 20 clusters - EMT hallmark genes", cluster_rows = T)
```
